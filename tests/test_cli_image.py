...
def test_image_show_progress_prints_percent(monkeypatch):
    fake = install_fake_client(monkeypatch)
    result = runner.invoke(
        cli.app, ["image", "show-progress", "--image-id", "compshareImage-custom-1"]
    )
    assert result.exit_code == 0
...
def test_image_delete_agent_risk_label(monkeypatch):
    install_fake_client(monkeypatch)
    result = runner.invoke(
        cli.app, ["image", "delete", "--image-id", "compshareImage-custom-1", "--agent"]
    )
    assert result.exit_code != 0
...
def test_image_delete_yes_calls_api(monkeypatch):
    fake = install_fake_client(monkeypatch)
    result = runner.invoke(
        cli.app, ["image", "delete", "--image-id", "compshareImage-custom-1", "--yes"]
    )
    assert result.exit_code == 0
